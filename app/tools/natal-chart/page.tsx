            ...
            <div className="card-luxury mb-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-[#7A6A58] mb-2 tracking-wide">Година (0-23)</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs text-[#7A6A58] mb-2 tracking-wide">Хвилина</label>
                    <input
                ...
                <div>
                  <label className="block text-xs text-[#7A6A58] mb-2 tracking-wide">Місто народження</label>
                  <select
                    value={form.city}
                ...
                <button type="submit" className="btn-primary w-full" disabled={loading}>
                  ...
                  {loading ? "Розраховуємо..." : "Розрахувати натальну карту"}
                </button>
              </form>
            </div>
          </AnimatedSection>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="card-luxury">
                    ...
                    <div className="space-y-4">
                      {[
                        { label: "Сонце ☉", value: degToSignName(result.sun), deg: result.sun },
                        { label: "Місяць ☽", value: degToSignName(result.moon), deg: result.moon },
                        { label: "Асцендент", value: degToSignName(result.asc), deg: result.asc },
                        { label: "МС (Зеніт)", value: degToSignName(result.mc), deg: result.mc },
                      ].map((item) => (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-[rgba(196,169,122,0.2)]">
                          <th className="text-left py-2 text-[#7A6A58] font-normal pr-4">Планета</th>
                          <th className="text-left py-2 text-[#7A6A58] font-normal pr-4">Знак</th>
                          <th className="text-left py-2 text-[#7A6A58] font-normal">Градус</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          return (
                            <tr key={key} className="border-b border-[rgba(196,169,122,0.08)] hover:bg-[rgba(196,169,122,0.04)]">
                              <td className="py-2.5 pr-4">
                                ...
                                <span className="text-[#1C1512]">{PLANET_NAMES_UA[i]}</span>
                              </td>
                              <td className="py-2.5 pr-4 text-[#5C4530]">
                                {SIGN_GLYPHS[signIdx]} {SIGNS_UA[signIdx]}
                              </td>
